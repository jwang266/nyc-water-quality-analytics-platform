...
router.get('/best', async (req, res) => {
    ...
    try {
        ...
        if (req.session.user) {
            userVoteBoroughId = await voteData.getUserVoteForWeek(req.session.user.id, weekStartObj);
            if (userVoteBoroughId) {
                userHasVoted = true;
                votingBoroughs = allBoroughs.filter(b => b._id.toString() === userVoteBoroughId.toString());
            }
        }
