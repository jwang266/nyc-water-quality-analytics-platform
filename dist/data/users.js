...
import { checkString, validateEmail, validatePassword, validateName, isValidId } from "../helper/helper.js";
const saltRounds = 10;
...
const exportedMethods = {
    async createUser(fname, lname, email, password, role = "user") {
        ...
        if (existing) {
            ...
            const reactivatedUser = await userCollection.findByIdAndUpdate(existing._id, { fname, lname, hashedPwd: hash, isDeleted: false, role }, { new: true });
    ...
    async updateUser(userId, fname, lname, email) {
        ...
        const updatedUser = await userCollection.findByIdAndUpdate(userId, { fname, lname, lowerEmail: newLowerEmail, updatedAt: new Date() }, { new: true });
        return fitForSession(updatedUser);
    },
    async removeUser(userId) {
        userId = isValidId(userId);
        const updated = await userCollection.findByIdAndUpdate(userId, { isDeleted: true }, { new: true });
        if (!updated)
            throw "User not found";
        return true;
    },
